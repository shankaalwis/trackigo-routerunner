...
export async function saveBus(bus: Bus): Promise<boolean> {
  ...
  // Then try Supabase
  const { error } = await (supabase as any).from("buses").upsert(bus);
...
export async function saveFleet(buses: Bus[]): Promise<boolean> {
  ...
  try {
    ...
    if (buses.length > 0) {
      const { error } = await (supabase as any).from("buses").insert(buses);
      return !error;
    }
    return true;
  } catch (e) {
...
export async function fetchConfig(): Promise<SchedulerConfig> {
  // Try Supabase
  const { data, error } = await (supabase as any).from("config").select("*").limit(1).single();
...
export async function saveConfig(config: SchedulerConfig): Promise<boolean> {
  ...
  const { error } = await (supabase as any).from("config").upsert({ id: 1, ...config });
